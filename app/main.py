...
def home(
    ...
    if q:
        like = f"%{q}%"
        stmt = stmt.where(
            Item.name.ilike(like) | Item.sku.ilike(like) | Item.notes.ilike(like)
        )
    if status:
...
def collection_overview(
    ...
    for item in items:
        ...
        if primary_purchase:
            ...
            vendor_name = (
                primary_purchase.vendor.name if primary_purchase.vendor else ""
            )
...
def item_detail(
    ...
    return templates.TemplateResponse(
        "item_detail.html",
        {"request": request, "item": item, "characters_csv": characters_csv},
    )
